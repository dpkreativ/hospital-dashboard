...
const DiagnosisHistory = ({
  ...
  const diastolics = diagnosisHistory
    .map((history) => history.blood_pressure.diastolic.value)
    .reverse();
  const systolics = diagnosisHistory
    .map((history) => history.blood_pressure.systolic.value)
    .reverse();
  const respiratoryRates = diagnosisHistory
    .map((history) => history.respiratory_rate.value)
    .reverse();
  const heartRates = diagnosisHistory
    .map((history) => history.heart_rate.value)
    .reverse();
  const temperatureRates = diagnosisHistory
    .map((history) => history.temperature.value)
    .reverse();

  const systolicAverage = Math.floor(
    systolics.reduce((acc, systolic) => acc + systolic, 0) / systolics.length
  );
  const diastolicAverage = Math.floor(
    diastolics.reduce((acc, diastolic) => acc + diastolic, 0) /
      diastolics.length
  );
  const respiratoryRateAverage = Math.floor(
    respiratoryRates.reduce((acc, rate) => acc + rate, 0) /
      respiratoryRates.length
  );
  const heartRateAverage = Math.floor(
    heartRates.reduce((acc, rate) => acc + rate, 0) / heartRates.length
  );
  const temperatureRateAverage = Math.floor(
    temperatureRates.reduce((acc, rate) => acc + rate, 0) /
      temperatureRates.length
  );
      ...
      <section className="grid grid-cols-1 grid-rows-1 lg:grid-cols-3 gap-y-12 bg-blue-50 px-5 pt-2 rounded-xl">
        ...
        <div className="flex flex-wrap">
          <div className="w-full grid grid-cols-1 grid-rows-3 text-md font-medium mb-4">
            ...
            <h3 className="py-1 flex items-center text-2xl font-medium">
              {systolicAverage}
            </h3>
            <div className="flex items-center">
              <ArrowUpIcon />
